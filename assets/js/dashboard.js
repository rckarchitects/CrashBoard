...
(function() {
    ...
    function renderEmailTile(container, data) {
        ...
        container.innerHTML = `
            ...
            ` : ''}
            <div class="tile-content-bottom-pad" aria-hidden="true"></div>
        `;
    ...
    function renderFlaggedEmailTile(container, data) {
        if (!data.connected) {
            container.innerHTML = `
                <div class="tile-placeholder">
                    <p>Connect Microsoft 365 to surface flagged emails</p>
                    <a href="/settings.php" class="tile-connect-btn">Connect Account</a>
                </div>
            `;
            return;
        ...
        if (data.error) {
            container.innerHTML = `
                <div class="tile-placeholder">
                    <p class="text-red-600">${escapeHtml(data.error)}</p>
                    <p class="text-sm mt-2">Use the refresh button to retry.</p>
                </div>
            `;
            return;
        }
        if (!data.email || data.totalFlagged === 0) {
            container.innerHTML = `
                <div class="empty-state">
                    ...
                    <p class="empty-state-text">No flagged emails in inbox</p>
                    <p class="text-xs opacity-75 mt-1">Flag emails in Outlook to see them here as reminders.</p>
                </div>
            `;
            return;
        }
        const email = data.email;
        const openLabel = email.webLink ? 'Open in Outlook' : 'View in Outlook';
        const openLink = email.webLink || 'https://outlook.office.com/mail/inbox';
        container.innerHTML = `
            <div class="flagged-email-tile">
                <div class="flagged-email-card">
                    <div class="flagged-email-meta">
                        <span class="flagged-email-from">${escapeHtml(email.from)}</span>
                        <span class="flagged-email-time">${escapeHtml(email.receivedTime)}</span>
                    </div>
                    <h4 class="flagged-email-subject">${escapeHtml(email.subject)}</h4>
                    ${email.preview ? `<p class="flagged-email-preview">${escapeHtml(email.preview)}</p>` : ''}
                    <div class="flagged-email-actions mt-3 flex flex-wrap gap-2">
                        <a href="${escapeHtml(openLink)}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2" style="background-color: var(--cb-primary);">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M10 6a2 2 0 012 2v2m0 4v6a2 2 0 01-2 2h-2a2 2 0 01-2-2V10a2 2 0 012-2h2"/>
                            </svg>
                            ${openLabel}
                        </a>
                    </div>
                </div>
                <p class="flagged-email-count text-xs opacity-75 mt-2">${data.totalFlagged} flagged email${data.totalFlagged !== 1 ? 's' : ''} in inbox · Refresh to see another</p>
            </div>
        `;
    }
    ...
    function renderFlaggedEmailCountTile(container, data) {
        if (!data.connected) {
            container.innerHTML = `
                <div class="tile-placeholder">
                    <p>Connect Microsoft 365 to view flagged email count</p>
                    <a href="/settings.php" class="tile-connect-btn">Connect Account</a>
                </div>
            `;
            return;
        ...
        container.innerHTML = `
            <div class="flagged-email-count-tile">
                <div class="flagged-email-count-number">${escapeHtml(count.toString())}</div>
                <div class="flagged-email-count-label">Flagged email${count !== 1 ? 's' : ''} in inbox</div>
            </div>
        `;
    }
    ...
    function renderOverdueTasksCountTile(container, data) {
        if (!data.connected) {
            container.innerHTML = `
                <div class="tile-placeholder">
                    <p>Connect Microsoft 365 to view overdue tasks count</p>
                    <a href="/settings.php" class="tile-connect-btn">Connect Account</a>
                </div>
            `;
            return;
        ...
        container.innerHTML = `
            <div class="flagged-email-count-tile">
                <div class="flagged-email-count-number">${escapeHtml(count.toString())}</div>
                <div class="flagged-email-count-label">Overdue task${count !== 1 ? 's' : ''}</div>
            </div>
        `;
    }
    ...
    function renderCalendarHeatmapTile(container, data) {
        if (!data.connected) {
            container.innerHTML = `
                <div class="tile-placeholder">
                    <p>Connect Microsoft 365 to view calendar heat map</p>
                    <a href="/settings.php" class="tile-connect-btn">Connect Account</a>
                </div>
            `;
            return;
        ...
        const maxCount = Math.max(1, data.maxCount || 0);
        const now = new Date();
        ...
        const cellsHtml = days.map(function (d, idx) {
            const level = intensityClass(d.count);
            const isToday = d.date === todayStr;
            const todayClass = isToday ? ' calendar-heatmap-today' : '';
            const title = d.count + ' event' + (d.count !== 1 ? 's' : '') + ' on this day';
        ...
        container.innerHTML = `
            <div class="calendar-heatmap-outer">
                <div class="calendar-heatmap-grid" role="grid" aria-label="Calendar heat map: event count by day (weekdays only)">
                    ${cellsHtml}
                </div>
        ...
        const grid = container.querySelector('.calendar-heatmap-grid');
        const popover = container.querySelector('.calendar-heatmap-popover');
        ...
        function showPopover(cellWrap, dayData) {
            ...
            const dateLabel = formatPopoverDate(dayData.date);
            let content = '<div class="calendar-heatmap-popover-title">' + escapeHtml(dateLabel) + '</div>';
            if (events.length === 0) {
            ...
            const rect = cellWrap.getBoundingClientRect();
        ...
        grid.querySelectorAll('.calendar-heatmap-cell[data-day-index]').forEach(function (cell) {
            const idx = parseInt(cell.getAttribute('data-day-index'), 10);
            const dayData = days[idx];
            cell.addEventListener('mouseenter', function () {
                showPopover(cell, dayData);
            });
    ...
    function renderCalendarNextTile(container, data) {
        ...
        if (!data.configured || !data.category) {
            container.innerHTML = `
                <div class="tile-placeholder">
                    <p>Set an Outlook category in Settings for this tile.</p>
                    <a href="/settings.php" class="tile-connect-btn">Settings</a>
                </div>
            `;
            return;
        }
        ...
        if (!data.event) {
            container.innerHTML = `
                <div class="calendar-next-empty">
                    <p class="calendar-next-empty-title">No upcoming events</p>
                    <p class="calendar-next-empty-sub">in category “${escapeHtml(data.category)}”</p>
                </div>
            `;
            return;
        }
        ...
        const e = data.event;
        const locationHtml = e.location ? `<div class="calendar-next-location">${escapeHtml(e.location)}</div>` : '';
        const daysLabel = formatDaysUntil(e.startDateTime);
        const mainDateLabel = e.startDate + (e.startTime && e.startTime !== 'All Day' ? ' · ' + e.startTime : '');

        let secondHtml = '';
        if (data.eventNext) {
            const e2 = data.eventNext;
            const secondDateLabel = e2.startDate + (e2.startTime && e2.startTime !== 'All Day' ? ' · ' + e2.startTime : '');
            secondHtml = `
                <div class="calendar-next-second">
    ...
    /**
     * Render next-event tile: single next calendar event with live countdown
     */
    function renderNextEventTile(container, data) {
        ...
        if (!data.event) {
            container.innerHTML = `
                <div class="calendar-next-empty">
                    <p class="calendar-next-empty-title">No upcoming events</p>
                    <p class="calendar-next-empty-sub">Your next calendar event will appear here</p>
                </div>
            `;
            return;
        }
        ...
        const e = data.event;
        const locationHtml = e.location ? `<div class="calendar-next-location">${escapeHtml(e.location)}</div>` : '';
        const dateTimeLabel = e.startDate + (e.startTime && e.startTime !== 'All Day' ? ' · ' + e.startTime : '');

        // Check if event is in progress (started but not ended)
        const now = new Date();
        const startMs = e.startDateTime ? new Date(e.startDateTime).getTime() : 0;
        const endMs = e.endDateTime ? new Date(e.endDateTime).getTime() : 0;
        const isInProgress = startMs > 0 && endMs > 0 && now >= startMs && now < endMs;
        ...
        // Live countdown: update every second when under 24h, else every minute
        const countdownEl = container.querySelector('.next-event-countdown');
        if (countdownEl && e.startDateTime) {
            function tick() {
                ...
                // Update class based on in-progress status
                if (isInProgress) {
                ...
                const text = formatCountdown(e.startDateTime);
                if (countdownEl.textContent !== text) countdownEl.textContent = text;
            }
            tick();
            const intervalMs = (startMs - Date.now()) < 24 * 60 * 60 * 1000 ? 1000 : 60000;
    ...
    /**
     * Format "X days" until a given ISO date/time (or "Today", "Tomorrow")
     */
    ...
    function renderTrainDeparturesTile(container, data) {
        ...
        const departuresHtml = departures.map(dep => {
            const statusClass = dep.isCancelled ? 'train-status-cancelled' : 
                               dep.isDelayed ? 'train-status-delayed' : 
                               'train-status-ontime';
            const statusIcon = dep.isCancelled ? '✕' : dep.isDelayed ? '⚠' : '✓';
            const platformPart = dep.platform ? `Plat ${escapeHtml(dep.platform)}` : '';
            /* Only show expected time in brackets when it's an actual time (e.g. 14:35), not status text like "On time" or "Delayed" */
            const isActualTime = dep.expectedDisplay && /^\d{1,2}:\d{2}$/.test(dep.expectedDisplay);
            const expectedPart = isActualTime && dep.expectedDisplay !== dep.scheduledDisplay && !dep.isCancelled
                ? ` (${escapeHtml(dep.expectedDisplay)})` : '';
            const destinationName = dep.destination && dep.destination.length > 0
                ? dep.destination[0].locationName || dep.destination[0].crs || ''
                : '';
        ...
        container.innerHTML = `
            ${routeHeader}
            <div class="train-departures-list">
                ${departuresHtml}
            </div>
        `;
    }
            ...
            <div class="notes-container">
                ...
                <div class="notes-footer">
                    ...
                    <div class="notes-footer-actions">
                        <button 
                            type="button"
                            id="notes-new-note-btn-${tileId}" 
                            class="notes-footer-icon-btn"
                        ...
                        <button 
                            ...
                            aria-label="Open in larger window"
                        >
                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button 
                            ...
                            class="notes-footer-icon-btn notes-save-btn"
                            title="${currentNoteId ? 'Update existing note' : 'Save note to list and clear'}"
                            aria-label="${currentNoteId ? 'Update existing note' : 'Save note to list and clear'}"
    ...
    function setupLinkBoardAddItem(container, tileElement) {
        const categories = [];
        container.querySelectorAll('.link-board-column').forEach(function (col) {
            const id = parseInt(col.dataset.categoryId, 10);
            const name = col.querySelector('.link-board-column-title');
            if (id && name) categories.push({ id: id, name: name.textContent });
        });
    ...
    function setupNotesNewNoteButton(tileId) {
        ...
        newNoteBtn.addEventListener('click', async function() {
            ...
            try {
                if (notes) {
                    const saveRes = await fetch(CONFIG.notesEndpoint, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': CONFIG.csrfToken,
                ...
                const newRes = await fetch(CONFIG.notesEndpoint, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': CONFIG.csrfToken,
    ...
    function closeNotesPopup(tileId) {
        ...
        if (modalSaveBtn && tileSaveBtn) {
            ...
            const title = tileSaveBtn.dataset.currentNoteId ? 'Update existing note' : 'Save note to list and clear';
    ...
    function setupTileRefreshTimer(tile) {
        ...
        // Skip tiles that shouldn't auto-refresh (claude, notes, bookmarks, link-board)
    ...
    /**
     * Update last update time display (all instances: desktop + mobile menu)
     */
    ...
    function setupHeaderRollover() {
        ...
        // Enable on screens up to 1920px (Full HD)
